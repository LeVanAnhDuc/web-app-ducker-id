...
- [ ] `pnpm type-check`; commit `feat(contact): add nullable userId owner field + index`.
...
- [ ] Test: submit có/không token → contact.userId set/null. `pnpm lint && pnpm type-check && pnpm test`. Commit `feat(contact): attach userId on authenticated submit`.
...
- [ ] Tests: `getMyContacts` chỉ trả owner=me; detail của mình ok, của người khác/absent → 404. `pnpm lint && pnpm type-check && pnpm test && pnpm build`. Commit `feat(contact): user my-contacts list + detail endpoints (owner-scoped)`.
...
- [ ] `pnpm exec tsc --noEmit`. Commit `feat(my-contacts): constants, types, requests`.
...
- [ ] i18n list + empty + announce (en+vi). `pnpm format && pnpm lint && pnpm exec tsc --noEmit`. Commit `feat(my-contacts): list view with search + status filter`.
...
- [ ] i18n detail + announce (en+vi). `pnpm format && pnpm lint && pnpm exec tsc --noEmit && pnpm build` (copy `.env.local` từ main cho build, xoá sau). Commit `feat(my-contacts): read-only detail page`.
