...
> **Date**: 2026-05-31
> **Status**: Approved (brainstorm) — đầu vào cho `pm-requirements` → `writing-plans`
...
1. `pm-requirements` → `specs/change-password/requirements.md` (User Stories + EARS AC + giữ mục **API Contract Sync** ở §9).
2. `superpowers:writing-plans` → chia task theo side (BE / FE), đặt tên rõ side.
3. Code theo `subagent-driven-development` + convention theo side (BE đọc `server/.claude/CLAUDE.md`, FE đọc `client/.claude/CLAUDE.md`).
